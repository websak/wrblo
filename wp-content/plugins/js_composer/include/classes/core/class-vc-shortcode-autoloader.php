...
class VcShortcodeAutoloader {
	 ...
	 *     that class to work
	 */
	public static function generateClassMap( $dirs ) { // phpcs:ignore:Generic.Metrics.CyclomaticComplexity.TooHigh, CognitiveComplexity.Complexity.MaximumComplexity.TooHigh
		$flat_map = [];
		foreach ( (array) $dirs as $dir ) {
			$directory = new RecursiveDirectoryIterator( $dir );
			$iterator = new RecursiveIteratorIterator( $directory );
