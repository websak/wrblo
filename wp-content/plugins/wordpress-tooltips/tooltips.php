...
$taglabels = array(
    'name' => 'Tags',
    'singular_name' => 'Tooltips Tags',
    'all_items' => 'All Tooltips Tags',
    'parent_item' => 'Parent Tooltips Tag',
    'edit_item' => 'Edit Tooltips Tag',
    'update_item' => 'Update Tooltips Tag',
    'add_new_item' => 'Add New Tooltips Tag',
    'new_item_name' => 'New Tooltips Tag'
);
...
function tooltips_menu() {
	...
	add_submenu_page('edit.php?post_type=tooltips', __('Directory Settings', 'member-directory'), __('Directory Settings', 'member-directory'), "manage_options", 'tooltipdirectorysetting', 'tooltipdirectorysettingfree'); // 8.1.9
...
function tooltips_loader_scripts()
{
	...
	*/
	...
	//8.5.3 enable jquery migreate for new user by default
	if (empty($seletEnableJqueryMigrate))
	{
	    $seletEnableJqueryMigrate = 'YES';
	}
	
	/* works well before 8.8.5
...
// 9.8.9
	...
	if ($selectcaseforleftcolumnglossarytitle == 'UPPERCASE')
	{
	?>
		<style type="text/css">
			...
			(
				{
					 ...
					 * 9.5.9 classes:' <?php echo $selectedTooltipStyle; ?> wordpress-tooltip-free qtip-rounded qtip-shadow <?php echo esc_attr($toolstipsAnimationClass); ?>'
   					 */
   					?>
   						classes:' <?php echo esc_attr($selectedTooltipStyle); ?> wordpress-tooltip-free qtip-rounded qtip-shadow <?php echo esc_attr($toolstipsAnimationClass); ?>'
    				},
    				position:
    				{
    					viewport: jQuery(window),
					...
					*/
					//{
						...
						if (!(empty($m_content)))
						{
							...
							// before 9.5.9 $m_keyword_result .= " toolTips('.classtoolTips$m_keyword_id','$m_content'); ";

							//9.5.9
							$m_keyword_result .= wp_kses_post(" toolTips('.classtoolTips$m_keyword_id','$m_content'); ");
						}
	...
	if (!(empty($m_result)))
	{
		...
		foreach ($m_result as $m_single)
		{
			...
			if ((!(empty($tooltsip_synonyms))) && (is_array($tooltsip_synonyms)) && (count($tooltsip_synonyms) > 0))
			{
				...
				foreach ($tooltsip_synonyms as $tooltsip_synonyms_single)
				{
					...
					else
					{
						...
						if ($onlyFirstKeyword == 'all')
						{
							...
							//before 8.9.1 $content1 = preg_replace("/([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])(".esc_attr($m_keyword).")(?![^<|^\[]*[>|\]])([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])/is","\\1"."<span class='tooltipsall tooltipsincontent classtoolTips".esc_attr($m_keyword_id)."'>"."\\2"."</span>"."\\3",$content);
							//8.9.1
							$content1 = preg_replace("/([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])(".esc_attr($m_keyword).")(?![^<|^\[]*[>|\]])([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])/is","\\1"."<span class='tooltipsall tooltipsincontent classtoolTips".esc_attr($m_keyword_id)."'>".$m_keyword_link."</span>"."\\3",$content);
						}
						...
						if ($onlyFirstKeyword == 'first')
						{
							...
							//8.9.1
							$content1 = preg_replace("/([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])(".esc_attr($m_keyword).")(?![^<|^\[]*[>|\]])([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])/is","\\1"."<span class='tooltipsall tooltipsincontent classtoolTips".esc_attr($m_keyword_id)."'>".$m_keyword_link."</span>"."\\3",$content,1);
						}
						...
						if ($content1 == $content)
						{
							...
							if ($onlyFirstKeyword == 'all')
							{
								...
								//7.9.3
								//$content1 = preg_replace("/([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])(".$m_keyword.")(?![^<|^\[]*[>|\]])([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])/is","\\1"."<span class='tooltipsall tooltipsincontent classtoolTips$m_keyword_id'>"."\\2"."</span>"."\\3",$content1);
								//8.9.1 $content1 = preg_replace("/([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])(".$m_keyword.")(?![^<|^\[]*[>|\]])([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])/is","\\1"."<span class='tooltipsall tooltipsincontent classtoolTips".esc_attr($m_keyword_id)."'>"."\\2"."</span>"."\\3",$content1);
								//8.9.1
								$content1 = preg_replace("/([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])(".$m_keyword.")(?![^<|^\[]*[>|\]])([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])/is","\\1"."<span class='tooltipsall tooltipsincontent classtoolTips".esc_attr($m_keyword_id)."'>".$m_keyword_link."</span>"."\\3",$content1);
							}
							...
							if ($onlyFirstKeyword == 'first')
							{
								...
								$content1 = preg_replace("/([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])(".$m_keyword.")(?![^<|^\[]*[>|\]])([^A-Za-z0-9àâåäąæÀÂĄÄÅÆçćÇĆéèëêęÉÈÊËĘłŁñńÑŃîïÎÏòôöœóøÔŒÓÖØšśŚŠùûüÙÛÜÿŸžźżŹŻŽбвгдёжзийлпфцчщъыьэюяß])/is","\\1"."<span class='tooltipsall tooltipsincontent classtoolTips".esc_attr($m_keyword_id)."'>".$m_keyword_link."</span>"."\\3",$content1,1);
							}
				...
				//!!! before 9.2.9 $m_single_keyword = str_replace('/','\/',$m_single['keyword']); //!!! 6.2.9
				// $content = preg_replace("/"."(".$m_single_keyword.")(?![^@@@@]*[####])/s",'@@@@'.$tooltip_unique_id.'####'."\\2",$content); //!!!new
				//!!! 7.9.5 this is not same of pro
				//!!! old before 9.2.3 $content = preg_replace("/"."(".$m_single_keyword.")(?![^@@@@]*[%%%%])/s",'@@@@'.$tooltip_unique_id.'%%%%'."\\2",$content); //!!!new
				/*
				//9.2.3 before 9.1.9
				$m_singlekeyword_search = $m_single_keyword;
				$m_singlekeyword_search = preg_quote($m_singlekeyword_search);
				*/
				//!!!9.2.9
				$m_single_keyword = $m_single['keyword'];
				$m_singlekeyword_search = $m_single_keyword;
				$m_singlekeyword_search = preg_quote($m_singlekeyword_search,'/');
...
//31.9.8 and 9.9.9
function tooltips_bootstrap_loader_scripts_free()
{
	wp_enqueue_style('bootstrap',  plugin_dir_url( __FILE__ ) . '/js/bootstrap/bootstrap.min.css');
...
{
    add_action('admin_enqueue_scripts', 'tooltips_bootstrap_loader_scripts_free');
}
...
//!!! 10.7.9
	...
	if ((!(empty($results))) && (is_array($results)) && (count($results) >0))
	{
		...
		foreach ($results as $single)
		{
			...
			if ($enabGlossaryIndexPage == 'YES')
			{
				//!!! before 9.8.9 $return_content .=	'<a href="'.get_permalink($single->ID).'">'.$single->post_title.'</a>';
				//9.8.9
				$case_single_post_title = $single->post_title;
				$case_single_post_title = functioncaseforleftcolumnglossarytitle_free($case_single_post_title);
				// before 10.4.9 $return_content .=	'<a href="'.esc_url(get_permalink($single->ID)).'">'.$case_single_post_title.'</a>';
				//end 9.8.9

				// start 10.4.9
				$enableGlossaryInNewWindow = get_option('enableGlossaryInNewWindow');
				if ($enableGlossaryInNewWindow == 'yes')
				{
					$return_content .=	'<a href="'.esc_url(get_permalink($single->ID)).'" target="_blank">'.$case_single_post_title.'</a>';
				}
				else
				{
					$return_content .=	'<a href="'.esc_url(get_permalink($single->ID)).'">'.$case_single_post_title.'</a>';
				}
				// end 10.4.9
			...
			{
				//!!! before 9.8.9 $return_content .=	$single->post_title;
				//9.8.9
				$case_single_post_title = $single->post_title;
				$case_single_post_title = functioncaseforleftcolumnglossarytitle_free($case_single_post_title);
				$return_content .=	$case_single_post_title;
		...
		if ((!(empty($disabletooltipinhtmltagarray))) && (is_array($disabletooltipinhtmltagarray)) && (count($disabletooltipinhtmltagarray) > 0))
		{
			...
			foreach ($disabletooltipinhtmltagarray as $disabletooltipinhtmltagSingle)
			{
				...
				jQuery(document).ready(function () {
					...
					*/
					?>
					jQuery('<?php echo esc_js($disabletooltipinhtmltagSingle);?> .tooltipsall').each
					(function()
					{
		...
		if ((!(empty($disabletooltipforclassandidsarray))) && (is_array($disabletooltipforclassandidsarray)) && (count($disabletooltipforclassandidsarray) > 0))
		{
			...
			foreach ($disabletooltipforclassandidsarray as $disabletooltipforclassandidsSingle)
			{
				...
				jQuery(document).ready(function () {
					...
					*/
					?>
					jQuery('<?php echo esc_js($disabletooltipforclassandidsSingle) ; ?> .tooltipsall').each
					(function()
					{
	...
	<table cellspacing="2" cellpadding="5" style="width: 100%;" class="form-table">
	    ...
	    <tr class="form-field">
	        <td>
				...
				*/
				?>
				<input type="text" id="toolstipbulletscreentag" name="toolstipbulletscreentag" value="<?php echo esc_attr($get_post_meta_value_for_this_page);  ?>">
...
function save_content_tooltips_bullet_free_screen_control_meta_box($post_id, $post, $update)
{
	...
	if(isset($_POST['toolstipbulletscreentag']) != "") {
		//before 8.2.3 $meta_box_checkbox_value = $_POST['toolstipbulletscreentag'];
	    $meta_box_checkbox_value = sanitize_text_field($_POST['toolstipbulletscreentag']);
...
// start 8.7.5
function footerdisabletooltipmaxbutonfree()
{
...
add_action('wp_footer', 'footerdisabletooltipmaxbutonfree');


//start 8.9.5
function footerFocusTooltipBoxFree()
{
...
add_action('wp_footer', 'footerFocusTooltipBoxFree');


function footerBlurTooltipBoxFree()
{
    $accesstooltipwithtabkey = get_option("accesstooltipwithtabkey");
...
add_action('wp_footer', 'footerBlurTooltipBoxFree',20);
//end 8.9.5

//9.8.9
function functioncaseforleftcolumnglossarytitle_free($posttitle)
{
	if (empty($posttitle))
	{
		return $posttitle;
	...
	if ($selectcaseforleftcolumnglossarytitle == 'DEFAULT')
	{
