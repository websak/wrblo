...
if ( ! is_admin() || ! current_user_can( 'manage_options' ) ) {
	wp_die(
		esc_html__( 'You do not have permission to access this page.', 'wp-analytify' ),
		'',
		array( 'response' => 403 )
	);
}
...
if ( ! defined( 'WPBRIGADE_SDK__DEV_MODE' ) || true !== WPBRIGADE_SDK__DEV_MODE ) {
	wp_die(
		esc_html__( 'Debug mode is not enabled.', 'wp-analytify' ),
		'',
		array( 'response' => 403 )
	);
}
...
$wpb_debug_msg_success = __( 'Successfully set the option.', 'wp-analytify' );
$wpb_debug_msg_error   = __( 'Option not set. Name must start with wpb_.', 'wp-analytify' );
?>
...
<p class="notice notice-warning" style="margin: 1em 0;" role="alert">
	<strong><?php esc_html_e( 'Admin-only debug page.', 'wp-analytify' ); ?></strong>
	<?php esc_html_e( 'This page shows sensitive data (keys, paths, user info). Do not share screenshots or leave unattended.', 'wp-analytify' ); ?>
</p>

<h2><?php esc_html_e( 'Actions', 'wp-analytify' ); ?></h2>
<table>
	<tbody>
		<tr>
			<td>
			...
			<td>
				...
				<button type="button" class="button" id="set_option_button"><?php esc_html_e( 'Set DB Option', 'wp-analytify' ); ?></button>
				<form id="set_option_form" method="post" style="display: none; margin-right: 10px;">
					...
					<div class="option-input-wrapper" style="display: inline-block;">
						<label for="option_name"><?php esc_html_e( 'Option Name (must start with wpb_):', 'wp-analytify' ); ?></label>
