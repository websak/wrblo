...
class GoodwishEdgeFramework {
    ...
    private function __construct() {
        $this->edgtOptions = GoodwishEdgeOptions::get_instance();
        $this->edgtMetaBoxes = GoodwishEdgeMetaBoxes::get_instance();
    }
    ...
    public function setSkin(GoodwishEdgeSkinAbstract $skinObject) {
        $this->skin = $skinObject;
    }
}

/**
 * Class GoodwishEdgeSkinManager
 *
...
/**
 * Class GoodwishEdgeSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class GoodwishEdgeSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(goodwish_edge_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function getMenuIcon($icon) {
        ...
        return goodwish_edge_get_skin_uri().'/assets/img/admin-logo-icon.png';
    }
    ...
    public function setShortcodeJSParams() { ?>
        <script>
            window.edgtSCIcon = '<?php echo goodwish_edge_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
...
class GoodwishEdgeOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $goodwish_edge_options)) {
            return $goodwish_edge_options[$key];
...
/*
   Class: GoodwishEdgeAdminPage
   A class that initializes Edge Admin Page
*/
class GoodwishEdgeAdminPage implements iGoodwishEdgeLayoutNode {
    ...
    function __construct($slug="", $title="", $icon = "") {
        $this->layout = array();
        $this->factory = new GoodwishEdgeFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iGoodwishEdgeRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: GoodwishEdgeMetaBoxes
...
/*
   Class: GoodwishEdgeMetaBox
   A class that initializes Edge Meta Box
*/
class GoodwishEdgeMetaBox implements iGoodwishEdgeLayoutNode {
    ...
    public function setTitle($label) {
        ...
        return $goodwish_edge_Framework->getSkin()->getSkinLabel().' '.$label;
    }
}

global $goodwish_edge_Framework;

$goodwish_edge_Framework = GoodwishEdgeFramework::get_instance();
