...
if(!function_exists('goodwish_edge_single_portfolio')) {
    function goodwish_edge_single_portfolio() {
        $portfolio_template = goodwish_edge_get_portfolio_single_type();
        ...
        if ($portfolio_template == 'gallery') {
            $params['holder_class'][] = 'edgtf-portfolio-gallery-' . goodwish_edge_options()->getOptionValue('portfolio_single_numb_columns');
        }

        goodwish_edge_get_module_template_part('templates/single/holder', 'portfolio', '', $params);
    }
}

if(!function_exists('goodwish_edge_get_portfolio_single_type')) {
    function goodwish_edge_get_portfolio_single_type() {
        return goodwish_edge_get_meta_field_intersect('portfolio_single_template');
    }
}

if(!function_exists('goodwish_edge_get_portfolio_single_media')) {
    function goodwish_edge_get_portfolio_single_media() {
	    ...
	    $single_type = goodwish_edge_get_portfolio_single_type();
        if($image_ids !== '') {
        ...
        if(is_array($videos) && count($videos)) {
            usort($videos, 'goodwish_edge_compare_portfolio_videos');
            foreach($videos as $video) {
                ...
                if(!empty($video['portfoliovideotype'])) {
                    ...
                    $media['video_url']   = goodwish_edge_portfolio_get_video_url($video);
...
if(!function_exists('goodwish_edge_portfolio_get_video_url')) {
    function goodwish_edge_portfolio_get_video_url($video) {
...
if(!function_exists('goodwish_edge_portfolio_get_media_html')) {
    function goodwish_edge_portfolio_get_media_html($media) {
        global $wp_filesystem;
        $params = array();

        //For adding image overlay in gallery template type
        $params['gallery'] = (goodwish_edge_get_portfolio_single_type() == 'gallery') ? true : false;

        if($media['type'] == 'image') {
            $params['lightbox'] = goodwish_edge_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
        ...
        if(in_array($media['type'], array('youtube', 'vimeo'))) {
            $params['lightbox'] = goodwish_edge_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
        ...
        goodwish_edge_get_module_template_part('templates/single/media/'.$media['type'], 'portfolio', '', $params);
    }
}

if(!function_exists('goodwish_edge_compare_portfolio_videos')) {
    /**
     ...
     */
    function goodwish_edge_compare_portfolio_videos($a, $b) {
...
if(!function_exists('goodwish_edge_compare_portfolio_options')) {
    /**
     ...
     * @return int result of comparison
     */
    function goodwish_edge_compare_portfolio_options($a, $b) {
...
if(!function_exists('goodwish_edge_portfolio_get_info_part')) {
    function goodwish_edge_portfolio_get_info_part($part) {
        $portfolio_template = goodwish_edge_get_portfolio_single_type();

        goodwish_edge_get_module_template_part('templates/single/parts/'.$part, 'portfolio', $portfolio_template, array('type'=>$portfolio_template));
    }
}