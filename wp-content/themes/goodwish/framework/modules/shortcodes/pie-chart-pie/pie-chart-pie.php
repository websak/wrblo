<?php
namespace GoodwishEdge\Modules\Shortcodes\PieCharts\PieChartDoughnut;
...
class PieChartPie implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ($i = 1; $i <= $this->chartFields; $i++) {
			...
			$chart_fields[] = array(
				'type' => 'textfield',
				'heading' => esc_html__('Chart Value ', 'goodwish') . $i,
				'param_name' => "chart_value_" . $i,
			);
			$chart_fields[] = array(
				'type' => 'colorpicker',
				'heading' => esc_html__('Chart Color ', 'goodwish') . $i,
				'param_name' => "chart_color_" . $i,
			);
			$chart_fields[] = array(
				'type' => 'textfield',
				'heading' => esc_html__('Chart Legend ', 'goodwish') . $i,
		...
		vc_map( array(
			'name' => esc_html__('Edge Pie Chart 2 (Pie)', 'goodwish'),
			'base' => $this->getBase(),
			...
			'params' => array_merge(
				array(
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__('Height','goodwish'),
						'param_name' => 'height',
					),
				),
				$chart_fields
