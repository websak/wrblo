...
if(!function_exists('goodwish_edge_styles')) {
    ...
    function goodwish_edge_styles() {
		...
		if(file_exists(EDGE_ROOT_DIR.'/assets/css/style_site.css') && goodwish_edge_is_css_folder_writable() && !is_multisite()) {
			wp_enqueue_style('goodwish-edge-style-site', EDGE_ASSETS_ROOT.'/css/style_site.css', $style_dynamic_deps_array, filemtime(EDGE_ROOT_DIR.'/assets/css/style_site.css')); //it must be included after woocommerce styles so it can override it
		}

        if(file_exists(EDGE_ROOT_DIR.'/assets/css/owl.carousel.min.css') && goodwish_edge_is_css_folder_writable() && !is_multisite()) {
			wp_enqueue_style('goodwish-edge-owl-carousel', EDGE_ASSETS_ROOT.'/css/owl.carousel.min.css', $style_dynamic_deps_array, filemtime(EDGE_ROOT_DIR.'/assets/css/owl.carousel.min.css')); //it must be included after woocommerce styles so it can override it
		}
...
if(!function_exists('goodwish_edge_scripts')) {
    ...
    function goodwish_edge_scripts() {
        ...
        wp_enqueue_script('owl-carousel', EDGE_ASSETS_ROOT . '/js/owl.carousel.min.js', array('jquery'), false, true);
        ...
        wp_enqueue_script('goodwish-edge-modules', EDGE_ASSETS_ROOT.'/js/modules.js', array('jquery'), false, true);
...
if(!function_exists('goodwish_edge_is_revolution_slider_installed')) {
    /**
     * Function that checks if give is installed
     * @return bool
     */
    function goodwish_edge_is_revolution_slider_installed() {
...
if ( ! function_exists( 'goodwish_edge_is_wp_gutenberg_installed' ) ) {
	/**
	 * Function that checks if WordPress 5.x with Gutenberg editor installed
	 * @return bool
	 */
	function goodwish_edge_is_wp_gutenberg_installed() {
...
if ( ! function_exists( 'goodwish_edge_is_elementor_installed' ) ) {
	/**
	 * Function that checks if Elementor plugin installed
	 * @return bool
	 */
	function goodwish_edge_is_elementor_installed() {
...
function register_acf_block_types() {
	...
	acf_register_block_type(array(
		...
		'render_template'	=> 'template-parts/blocks/campigns.php',
		'category'			=> 'formatting',
		'icon'				=> 'layout'
	));
	...
	acf_register_block_type(array(
		...
		'description'		=> __('Recent Campaigns'),
		'render_template'	=> 'template-parts/blocks/recent-campaigns.php',
		'category'			=> 'formatting',
		'icon'				=> 'layout'
	));
	...
	acf_register_block_type(array(
		'name'				=> 'fullwidth-content-block',
		'title'				=> __('Fullwidth Content'),
		'description'		=> __('Fullwidth Content'),
		'render_template'	=> 'template-parts/blocks/card-full-width-content.php',
		'category'			=> 'formatting',
		'icon'				=> 'layout'
	));
	...
	acf_register_block_type(array(
		...
		'title'				=> __('Form'),
		'description'		=> __('Form'),
		'render_template'	=> 'template-parts/blocks/card-form.php',
		'category'			=> 'formatting',
		'icon'				=> 'layout'
	));
	...
	acf_register_block_type(array(
		...
		'render_template'	=> 'template-parts/blocks/card-call-to-action.php',
		'category'			=> 'formatting',
		'icon'				=> 'layout'
	));
	...
	acf_register_block_type(array(
		...
		'render_template'	=> 'template-parts/blocks/card-accordion.php',
		'category'			=> 'formatting',
		'icon'				=> 'layout'
	));
...
// Intercept checkout page and redirect back to cart if coming from donation
add_action('template_redirect', 'catch_donation_checkout_redirect', 1);
function catch_donation_checkout_redirect() {
    if (is_checkout() && !is_wc_endpoint_url('order-received')) {
        $referer = wp_get_referer();
        
        // If we just came from a product/shop page, redirect to cart
        if ($referer && (strpos($referer, '/product/') !== false || strpos($referer, '/shop') !== false)) {
...
// Remove max amount restriction for donations
add_filter('wc_donation_amount_max', '__return_false', 999);
add_filter('wc_donation_max_price', '__return_false', 999);

// Override the validation check
add_filter('wc_donation_validate_max_amount', '__return_true', 999);


// Change donation input placeholder text
add_action('wp_footer', 'change_donation_placeholder');
function change_donation_placeholder() {
    ?>
    <script>
    jQuery(document).ready(function($) {
        $('input[name*="donation"], input[class*="donation"], input[id*="donation"]').attr('placeholder', 'Enter Amount');
    });
    </script>
    <?php
}