...
if( ! class_exists('GoodwishEdgeSidebar') ){
	
	class GoodwishEdgeSidebar{
		...
		function __construct(){
			...
			$this->title = esc_html__('Custom Widget Area','goodwish');
		...
		function load_assets(){
			...
			wp_enqueue_script('edgt-sidebar' , EDGE_ROOT . '/framework/admin/assets/js/edgtf-sidebar.js');
			wp_enqueue_style( 'edgt-sidebar' , EDGE_ROOT . '/framework/admin/assets/css/edgtf-sidebar.css');
		}
		...
		function template_add_widget_field(){
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_html__('Enter Name of the new Widget Area', 'goodwish')."' name='edgt-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_html__('Add Widget Area', 'goodwish')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function add_sidebar_area(){
			if(!empty($_POST['edgt-add-widget'])){
				...
				$name = rtrim($this->get_name(sanitize_text_field($_POST['edgt-add-widget'])));
		...
		function register_custom_sidebars(){
			...
			$args = apply_filters('goodwish_edge_custom_widget_args', $args);
